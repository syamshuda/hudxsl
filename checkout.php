<?php
// /checkout.php (Versi FINAL dengan perbaikan total pada fitur alamat, voucher, dan kelurahan)
$page_title = "Checkout";
                ...
                <div id="physical-section">
                    <div class="card shadow-sm mb-4">
                        ...
                        <div class="card-body">
                            ...
                            <div class="mb-3"><label class="form-label">Kelurahan/Desa</label><input type="text" class="form-control" name="kelurahan_desa" placeholder="Contoh: Mlajah" required></div>
...
$(document).ready(function() {
    ...
    function loadVouchers() {
        ...
        container.html('<p class="text-center text-muted">Memuat voucher...</p>');
    ...
    if (adaProdukFisik) {
        if (!kecamatanAsal) {
            ...
            $.getJSON(`${BASE_URL}/api_alamat.php?get=tujuan_provinsi&kecamatan_asal=${encodeURIComponent(kecamatanAsal)}`)
                .done(function(data) {
            ...
            provSelect.on('change', function() {
                const prov = $(this).val();
                kotaSelect.empty().append('<option value="">Memuat...</option>').prop('disabled', true);
                kecSelect.empty().append('<option value="">Pilih kota/kab.</option>').prop('disabled', true);
                if (prov) {
                    $.getJSON(`${BASE_URL}/api_alamat.php?get=tujuan_kota&kecamatan_asal=${encodeURIComponent(kecamatanAsal)}&provinsi=${encodeURIComponent(prov)}`, function(data) {
                        kotaSelect.empty().append('<option value="">-- Pilih Kota/Kab. --</option>');
