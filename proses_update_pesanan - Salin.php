<?php
// /penjual/proses_update_pesanan.php
                                    ...
                                    if ($stmt_verify->get_result()->num_rows == 0) {
                                            throw new Exception("Anda tidak memiliki akses ke pesanan ini.");
                                                    ...
                                                    $stmt_verify->close();
                                                            ...
                                                            // --- PERBAIKAN: Logika di sini hanya untuk mengubah status menjadi 'dikirim' ---
                                                                // Logika penyelesaian pesanan dan transfer dana akan ditangani oleh pesanan_diterima.php
                                                                    $nomor_resi = trim($_POST['nomor_resi']);
                                                                        
                                                                            // Untuk produk fisik, nomor resi wajib diisi KECUALI jika itu pesanan COD
                                                                                $pesanan_info_res = $koneksi->query("SELECT metode_pembayaran FROM pesanan WHERE id = $pesanan_id");
                                                                                    $metode_pembayaran = $pesanan_info_res->fetch_assoc()['metode_pembayaran'];

                                                                                        if (empty($nomor_resi) && $metode_pembayaran !== 'COD') {
                                                                                                throw new Exception("Nomor resi wajib diisi untuk pengiriman non-COD.");
                                                                                                    }
                                                                                                        
                                                                                                            // Update status pesanan, nomor resi, dan tanggal kirim
                                                                                                                $stmt_update = $koneksi->prepare("UPDATE pesanan SET status_pesanan = 'dikirim', nomor_resi = ?, tanggal_dikirim = NOW() WHERE id = ?");
                                                                                                                    $stmt_update->bind_param("si", $nomor_resi, $pesanan_id);
