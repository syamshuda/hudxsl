<?php
// /penjual/laporan_kurir.php (Versi FINAL dengan Rincian Gaji & Filter Kurir)
...
// Ambil daftar kurir untuk dropdown filter
...
// Gabungkan semua logika query menjadi satu untuk konsistensi
$base_query = "
    FROM pesanan p 
...
<div class="row">
    <div class="col-lg-3 col-md-6 mb-4">
        <div class="card bg-info text-white h-100">
            <div class="card-body">
                <h6>Total Pengantaran</h6>
                <h4 class="fw-bold"><?php echo number_format($stats['total_pengantaran'] ?? 0); ?></h4>
    ...
    <div class="col-lg-3 col-md-6 mb-4">
        <div class="card bg-primary text-white h-100">
            <div class="card-body">
                <h6>Total Gaji Kurir</h6>
                <h4 class="fw-bold">Rp <?php echo number_format($stats['total_gaji'] ?? 0, 0, ',', '.'); ?></h4>
                <small>Total upah dari pengantaran.</small>
            </div>
        </div>
    </div>
    <div class="col-lg-3 col-md-6 mb-4">
        <div class="card bg-success text-white h-100">
            <div class="card-body">
                ...
                <h4 class="fw-bold">Rp <?php echo number_format($stats['total_dibayar'] ?? 0, 0, ',', '.'); ?></h4>
                <small>Total gaji yang sudah Anda bayarkan.</small>
            </div>
        </div>
    </div>
    <div class="col-lg-3 col-md-6 mb-4">
        <div class="card bg-danger text-white h-100">
            <div class="card-body">
                ...
                <h4 class="fw-bold">Rp <?php echo number_format($stats['total_tagihan'] ?? 0, 0, ',', '.'); ?></h4>
                <small>Total gaji yang perlu dibayarkan.</small>
            </div>
        </div>
    </div>
</div>
...
<div class="card shadow-sm">
    <div class="card-header">
        <h5 class="mb-0">Rincian Pengantaran</h5>
    </div>
    <div class="card-body">
            ...
            <div class="row g-3 align-items-end">
                ...
                <div class="col-md-3">
                     ...
                     <select name="kurir_id" id="kurir_id" class="form-select">
                        ...
                        <?php while($kurir = $result_kurir_list->fetch_assoc()): ?>
                            <option value="<?php echo $kurir['id']; ?>" <?php if($filter_kurir_id == $kurir['id']) echo 'selected'; ?>>
                ...
                <div class="col-md-2">
                     ...
                     <select name="status_pembayaran" id="status_pembayaran" class="form-select">
                        <option value="semua" <?php if($filter_status_pembayaran == 'semua') echo 'selected'; ?>>Semua</option>
                        <option value="Belum Dibayar" <?php if($filter_status_pembayaran == 'Belum Dibayar') echo 'selected'; ?>>Belum Dibayar</option>
                        <option value="Sudah Dibayar" <?php if($filter_status_pembayaran == 'Sudah Dibayar') echo 'selected'; ?>>Sudah Dibayar</option>
                     </select>
                </div>
                <div class="col-md-auto">
                    <button type="submit" class="btn btn-primary">Filter</button>
                    <a href="laporan_kurir.php" class="btn btn-secondary">Reset</a>
                </div>
            </div>
        </form>
            ...
            <table class="table table-hover align-middle">
                <thead class="table-light">
                    <tr>
                        ...
                        <th>No. Pesanan</th>
                        <th>Nama Kurir</th>
                        <th class="text-end">Ongkir (Pembeli)</th>
                        <th class="text-end">Gaji (Kurir)</th>
                        <th class="text-end">Laba (Penjual)</th>
                ...
                <tbody>
                    <?php if ($result_rincian->num_rows > 0): ?>
                        <?php while($item = $result_rincian->fetch_assoc()): 
                            $laba_penjual = $item['biaya_ongkir'] - $item['gaji_kurir'];
                        ?>
                            <tr id="row-<?php echo $item['pesanan_id']; ?>">
                                ...
                                <td class="text-end <?php echo ($laba_penjual >= 0) ? 'text-success' : 'text-danger'; ?>">
                                    Rp <?php echo number_format($laba_penjual); ?>
                                </td>
                                <td class="text-center">
                                    <span id="status-<?php echo $item['pesanan_id']; ?>" class="badge <?php echo $item['status_pembayaran_kurir'] == 'Sudah Dibayar' ? 'bg-success' : 'bg-warning text-dark'; ?>">
                                        <?php echo htmlspecialchars($item['status_pembayaran_kurir']); ?>
                                    </span>
                                </td>
                                <td class="text-center">
                        ...
                        <?php endwhile; ?>
                    <?php else: ?>
                        <tr><td colspan="8" class="text-center p-4">Tidak ada data pengantaran untuk filter yang dipilih.</td></tr>
                    <?php endif; ?>
                </tbody>
            </table>
        </div>
