<?php
// /includes/functions.php (Versi Final dengan Semua Beban Voucher Ditanggung Penjual)
...
if (!function_exists('selesaikanPesananDanTransferDana')) {
    function selesaikanPesananDanTransferDana($pesanan_id, $koneksi) {
        try {
            // 1. Ambil data pesanan yang krusial
            ...
            // 2. Ambil persentase komisi platform
            ...
            // 3. Ambil total harga produk per toko dalam pesanan ini
            ...
            // 4. Loop untuk setiap toko dalam pesanan
            while($toko = $result_items->fetch_assoc()) {
                ...
                // Cek jenis voucher dan hitung bebannya
                ...
                // 5. Hitung pendapatan bersih penjual & komisi admin
                // Pendapatan bersih = (Total Produk - Diskon Langsung - Cashback)
                ...
                // 6. Proses dana untuk Penjual
                if ($metode_pembayaran !== 'COD') {
                    // Dana yang diterima = (Pendapatan Bersih - Komisi) + (Ongkir Aktual - Potongan Ongkir)
                ...
                // 7. Proses dana untuk Pembeli (jika ada cashback)
                if ($jumlah_cashback > 0) {
                    // (Logika ini sudah benar dari sebelumnya, tidak perlu diubah)
                    ...
                    if (!$stmt_log_cashback->execute()) throw new Exception("Gagal mencatat riwayat cashback.");
                    $stmt_log_cashback->close();
                }
            }
            $stmt_items->close();

            // 8. Catat komisi untuk Admin
